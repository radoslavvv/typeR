  ...
  reducers: {
    ...
    deleteCurrentLetter: (state) => {
      if (state.letterIndex === 0 && state.wordIndex != 0) {
        const newLetterIndex = state.currentVisibleWords[state.wordIndex - 1]?.letters?.length;
    ...
    setCurrentLetterColor: (state, action: PayloadAction<LetterColor>) => {
      state.currentVisibleWords[state.wordIndex].letters[state.letterIndex] = {
        ...state.currentVisibleWords[state.wordIndex].letters[state.letterIndex],
    ...
    moveToNextWord: (state) => {
      state.currentVisibleWords[state.wordIndex]?.letters?.map((l: Letter) => {
      ...
      state.currentVisibleWords[state.wordIndex].letters = state.currentVisibleWords[state.wordIndex]?.letters?.filter((l: Letter) => {
        return l.key;
      });
    ...
    moveToNextPage: (state) => {
      if (state.wordIndex > 0) {
        const startIndex: number = state.wordIndex * state.pageNumber + state.wordIndex;
        // const endIndex: number = startIndex + state.wordsPerPage;

        state.currentVisibleWords = [...state.allWords.slice(startIndex, startIndex + 75)];
...
export const { setAllWords, moveToNextLetter, setCurrentLetterColor, deleteCurrentLetter, moveToPreviousWord, moveToNextWord, setShiftIsPressed, moveToNextPage, resetWriter } = writerSlice.actions;
