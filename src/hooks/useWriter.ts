...
import { deleteCurrentLetter, moveToNextLetter, moveToNextPage, moveToNextWord, setAllWords, setShiftIsPressed, setCurrentLetterColor } from "../redux/features/writer/writerSlice";
...
const useWriter = () => {
  ...
  const { currentVisibleWords, letterIndex, wordIndex, shiftIsPressed } = useSelector((state: any) => state.writer);
  ...
  const handleLetterInput = (pressedKey: string): void => {
    ...
    if (currentVisibleWords[wordIndex]?.letters[letterIndex]?.key === pressedKey) {
